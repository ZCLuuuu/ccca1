...
# treat each line as string, ignore json format
def analyze(twt_string):
    # get date time
    date_time_str = re.search(r'"created_at":"(.*?)"', twt_string).group(1)
    ...
    # get sentiment
    sentiment_str = re.search(r'"sentiment":(.*?),', twt_string)
    if not sentiment_str:
        sentiment = 0
    else:
        sentiment_str = sentiment_str.group(1)
        if sentiment_str.startswith("{"):  # "sentiment":{"score":0, ...}
            sentiment = float(sentiment_str.split(":")[1])
        else:  # "sentiment":0.5384615384615384}
            sentiment = float(sentiment_str[:-1])
            # print(sentiment)
